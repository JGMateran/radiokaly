(function () {
  ...
  // Obtener Datos desde NexoStreaming API
  const getDataFromStreamAfrica = async (artist, title, defaultArt, defaultCover) => {
    ...
    const API_URL = `https://radioapi.lat/search?query=${encodeURIComponent(text)}&service=spotify`;
    const response = await fetch(API_URL);
    if (title === 'nexostreaming.com' || response.status === 403) {
      const results = {
        title,
        artist,
        art: defaultArt,
